...
class G2Class:
    ...
    def __init__(self, cnt, approx, G2ID):
        ...
        if len(approx[G2ID]) == 4 or 8:
            #Loops over points
            numOfPoints = len(approx[G2ID])
            for pointsIndex in range(0, numOfPoints):
                coordArray = approx[G2ID][pointsIndex][0]
...
#        cv2.imwrite('t.jpg',tempimage)
    ...
    def findAvgs(self):
        if len(self.RTpts) == 2:
            ...
            self.avgPts.append((int)(self.RTpts[0]))
            self.avgPts.append((int)(self.LTpts[0]))
            self.avgPts.append((int)(self.LBpts[0]))
            self.avgPts.appemd((int)(self.RBpts[0]))
